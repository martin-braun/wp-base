...
(function($) {
	...
	$.wbcr_factory_clearfy_234.filters = $.wbcr_factory_clearfy_234.filters || $.wfactory_443.filters;
	//todo: Переопредление для совместимости со старыми версиями плагинов.
	$.wbcr_factory_clearfy_234.hooks = $.wbcr_factory_clearfy_234.hooks || $.wfactory_443.hooks;
	...
	$.wbcr_factory_clearfy_234.app = $.wbcr_factory_clearfy_234.app || {
		...
		showNotice: function(message, type) {
			...
			$([document.documentElement, document.body]).animate({
				 ...
				 * Пример регистрации хука $.wfactory_443.hooks.add('wbcr/factory_clearfy_234/updated',
				 * function(noticeId) {});
				 * @param {string} noticeId - id уведомления
				 */
				$.wfactory_443.hooks.run('wbcr/factory_clearfy_234/showed_notice', [noticeId]);
				$.wfactory_443.hooks.run('wbcr/clearfy/showed_notice', [noticeId]);
			});
		...
		hideNotice: function(noticeId) {
			...
			el.fadeOut(500, function(e) {
				 ...
				 * @param {string} noticeId - id уведомления
				 */
				$.wfactory_443.hooks.run('wbcr/factory_clearfy_234/hidded_notice', [noticeId]);
				$.wfactory_443.hooks.run('wbcr/clearfy/hidded_notice', [noticeId]);
			});
		},
	...
	$.wfactory_443.hooks.add('core/components/pre_activate', function(button) {
	...
	$.wfactory_443.hooks.add('core/components/deactivated', function(button, data, response) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !$('#WBCR').length ) {
			return false;
		}
	...
	$.wfactory_443.hooks.add('core/components/deleted', function(button) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !$('#WBCR').length ) {
			return false;
		}
	...
	$.wfactory_443.hooks.add('core/components/activation_error', function(plugin, button, response) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
	...
	$.wfactory_443.hooks.add('core/components/update_error', function(button, data, response) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !($('#WBCR').length && $.wbcr_factory_clearfy_234) ) {
			return false;
		}
	...
	$.wfactory_443.hooks.add('core/components/activated', function(button, data, response) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !$('#WBCR').length ) {
			return false;
		}
	...
	$.wfactory_443.hooks.add('core/components/ajax_error', function(xhr, ajaxOptions, thrownError) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !($('#WBCR').length && $.wbcr_factory_clearfy_234) ) {
			return false;
		}
