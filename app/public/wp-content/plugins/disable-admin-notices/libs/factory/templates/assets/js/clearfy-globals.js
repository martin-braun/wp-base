 ...
 * и самого Clearfy.
 *
 * $.wbcr_factory_templates_100.app - методы для работы с приложением. Скрыть, показать уведомления.
 * $.wbcr_factory_templates_100.hooks - это иммитация хуков и фильтров аналогично тем, что используются в Wordpress
 *
...
(function($) {
	...
	if( !$.wbcr_factory_templates_100 ) {
		$.wbcr_factory_templates_100 = {};
	}

	//todo: Переопредление для совместимости со старыми версиями плагинов.
	$.wbcr_factory_templates_100.filters = $.wbcr_factory_templates_100.filters || $.wfactory_450.filters;
	//todo: Переопредление для совместимости со старыми версиями плагинов.
	$.wbcr_factory_templates_100.hooks = $.wbcr_factory_templates_100.hooks || $.wfactory_450.hooks;

	$.wbcr_factory_templates_100.app = $.wbcr_factory_templates_100.app || {
		/**
		...
		showNotice: function(message, type) {
			...
			$([document.documentElement, document.body]).animate({
				 ...
				 * Пример регистрации хука $.wfactory_450.hooks.add('wbcr/factory_templates_100/updated',
				 * function(noticeId) {});
				 * @param {string} noticeId - id уведомления
				 */
				$.wfactory_450.hooks.run('wbcr/factory_templates_100/showed_notice', [noticeId]);
		...
		hideNotice: function(noticeId) {
			...
			el.fadeOut(500, function(e) {
				 ...
				 * @param {string} noticeId - id уведомления
				 */
				$.wfactory_450.hooks.run('wbcr/factory_templates_100/hidded_notice', [noticeId]);
	...
	$.wfactory_450.hooks.add('core/components/activation_error', function(plugin, button, response) {
		...
		if( response.data && response.data.error_message ) {
			$.wbcr_factory_templates_100.app.showNotice(response.data.error_message, 'danger');
		}
	});
	...
	$.wfactory_450.hooks.add('core/components/ajax_error', function(xhr, ajaxOptions, thrownError) {
		...
		$.wbcr_factory_templates_100.app.showNotice('Error: [' + thrownError + '] Status: [' + xhr.status + '] Error massage: [' + xhr.responseText + ']', 'danger');
	});
