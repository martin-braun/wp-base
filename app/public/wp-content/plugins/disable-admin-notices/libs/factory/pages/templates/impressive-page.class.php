...
if( !class_exists('Wbcr_FactoryPages442_ImpressiveThemplate') ) {
	/**
	 * Class Wbcr_FactoryPages442_ImpressiveThemplate
	 */
	abstract class Wbcr_FactoryPages442_ImpressiveThemplate extends Wbcr_FactoryPages442_AdminPage {
		...
		{
			$this->menuIcon = FACTORY_PAGES_442_URL . '/templates/assets/img/webcraftic-plugin-icon.png';
			//$allow_multisite = apply_filters('wbcr_factory_443_core_admin_allow_multisite', false);
			...
			$this->title_plugin_action_link = __('Settings', 'wbcr_factory_pages_442');
		...
		public function assets($scripts, $styles)
		{
			...
			$this->styles->add(FACTORY_PAGES_442_URL . '/templates/assets/css/impressive.page.template.css');
		}
		...
		{
			/*if( WP_CACHE ) {
				$this->printWarningNotice(__("It seems that a caching/performance plugin is active on this site. Please manually invalidate that plugin's cache after making any changes to the settings below.", 'wbcr_factory_pages_442'));
			}*/
		...
		private function showActionsNotice()
		{
			$notices = [
				[
					...
					'type' => 'success',
					'message' => __('The settings have been updated successfully!', 'wbcr_factory_pages_442') . (WP_CACHE ? '<br>' . __("It seems that a caching/performance plugin is active on this site. Please manually invalidate that plugin's cache after making any changes to the settings below.", 'wbcr_factory_pages_442') : '')
				]
			];
				...
				<div class="wbcr-factory-control">
					...
					<?php if( $this->type == 'options' ): ?>
						<input name="<?= $this->plugin->getPluginName() ?>_save_action"
						       ...
						       value="<?php _e('Save', 'wbcr_factory_pages_442'); ?>">
		...
		{
			$form = new Wbcr_FactoryForms440_Form([
				'scope' => rtrim($this->plugin->getPrefix(), '_'),
				'name' => $this->getResultId() . "-options"
			], $this->plugin);

			$form->setProvider(new Wbcr_FactoryForms440_OptionsValueProvider($this->plugin));
			...
			if( isset($_POST[$this->plugin->getPluginName() . '_save_action']) ) {
				...
				if( !current_user_can('administrator') && !current_user_can($this->capabilitiy) ) {
					wp_die(__('You do not have permission to edit page.', 'wbcr_factory_pages_442'));
				}
			...
			<div id="WBCR" class="wrap">
				<div class="wbcr-factory-pages-442-impressive-page-template factory-bootstrap-443 factory-fontawesome-000">
