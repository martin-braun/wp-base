 ...
 */
// @formatter:off
if ( ! class_exists( 'Wbcr_Factory461_Requirements' ) ) {
	class Wbcr_Factory461_Requirements {
		...
		public function show_notice() {
			...
			echo '<div class="notice notice-error"><p>' . esc_html($this->get_notice_text()) . '</p></div>';
		}
		...
		protected function get_php_incompat_text() {
			return sprintf( __( 'You need to update the PHP version to %s or higher!', 'wbcr_factory_461' ), $this->required_php_version );
		}
		...
		protected function get_wp_incompat_text() {
			return sprintf( __( 'You need to update WordPress to %s or higher!', 'wbcr_factory_461' ), $this->required_wp_version );
		}
		...
		protected function get_plugin_already_activate_text() {
			return sprintf( __( 'Plugin %s is already activated, you are trying to activate it again.', 'wbcr_factory_461' ), $this->plugin_title );
		}
	}
}
// @formatter:on